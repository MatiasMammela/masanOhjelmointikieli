...
token_t *expect_assert(const char c)
{
    token_t *t;
    if (head != NULL && head->next->value[0] == c)
    {
        t = head;
        head = head->next;
        return head;
...
token_t *expect_operation()
{
    token_t *t;
    if (head != NULL && is_operation(head->next))
    {
        t = head;
        head = head->next;
        return head;
...
token_t *expect_assert_token_type(token_type_t type)
{
    token_t *t;
    if (head != NULL && head->next->type == type)
    {
        t = head;
        head = head->next;
        return head;
...
{
    token_t *t;
    if (head != NULL && head->next->type == TOKEN_TYPE)
    {
        t = head;
        head = head->next;
        return head;
...
token_t *accept_assert(const char c)
{
    token_t *t;
    if (head != NULL && head->next->value[0] == c)
    {
...
token_t *accept_assert_token_type(token_type_t type)
{
    token_t *t;
    if (head != NULL && head->next->type == type)
    {
